<?php 
include("/var/www/html/cred.inc.php");
$conn = mysqli_connect('127.0.0.1',$my_username,$my_password,$my_db_name) or die("Failed to query database".mysqli_error());
$user_id = $_POST['user_id'];
$word_id = $_POST['word_id'];

$sql = "SELECT * FROM fav WHERE user_id='".$user_id."' AND word_id='".$word_id."'"; 
$result=mysqli_query($conn,$sql) or die (mysqli_error($conn).$sql);
$count = mysqli_num_rows($result);

$sql1="SELECT * from trending where id ='".$word_id."'";
$query1=mysqli_query($conn,$sql1)or die (mysqli_error($conn));
$row=mysqli_fetch_array($query1);
$no_of_likes=$row['no_of_likes'];

if($count > 0){
  $favrow = mysqli_fetch_array($result);
  $status = $favrow['status'];
  if($status == '0'){
    $no_of_likes++;
    $sql2 = "UPDATE fav SET status='1' WHERE user_id='".$user_id."' AND word_id='".$word_id."'";
    $sql3 = "UPDATE trending SET no_of_likes='".$no_of_likes."' WHERE id='".$word_id."'";
  }
  if($status == '1'){
    $no_of_likes--;
    $sql2 = "UPDATE fav SET status='0' WHERE user_id='".$user_id."' AND word_id='".$word_id."'";
    $sql3 = "UPDATE trending SET no_of_likes='".$no_of_likes."' WHERE id='".$word_id."'";
  }
  $bool = mysqli_query($conn,$sql2)or die (mysqli_error($conn));
  $bool2 = mysqli_query($conn,$sql3)or die (mysqli_error($conn));
}else{
  $sql2 = "INSERT INTO `fav` (`user_id`,`word_id`,`status`,`table_id`) VALUES ('".$user_id."','".$word_id."','1','0')";
  $no_of_likes++;
  $sql3 = "UPDATE trending SET no_of_likes='".$no_of_likes."' WHERE id='".$word_id."'";
  $bool = mysqli_query($conn,$sql2)or die (mysqli_error($conn));
  $bool2 = mysqli_query($conn,$sql3)or die (mysqli_error($conn));
}
?>
